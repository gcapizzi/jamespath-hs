    ...
    context "with a function call" $ do
      ...
      describe "max_by" $
        it "returns the value for which the expression returns the maximum number or string" $ do
          ...
          search "max_by(@, &[0])" "[[1], [false], [3]]" `shouldBe` Left "max_by: invalid type of values"
          search "max_by(@, &[0])" "[[\"a\"], [{}], [\"c\"]]" `shouldBe` Left "max_by: invalid type of values"
          search "max_by(@, &[0])" "[[\"a\"], [2], [\"c\"]]" `shouldBe` Left "max_by: invalid type of values"
